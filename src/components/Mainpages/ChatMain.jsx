...
const ChatMain = () => {
  ...
  // Fetch recent chats and set up real-time listener
  useEffect(() => {
    const fetchRecentChats = () => {
      if (!userID) return;
      ...
      try {
        setIsLoading(true);
        const recentChatsRef = collection(db, "messages", userID, "recent_chats");
        const recentChatsQueryObj = query(recentChatsRef, orderBy("sortTime", "desc"));
        ...
        const unsubscribe = onSnapshot(
          ...
          async (snapshot) => {
            ...
            if (recentChatsIds.length > 0) {
              ...
              const usersDataFetched = await Promise.all(userPromises);
              setUsersData(usersDataFetched.filter((user) => user !== null));
            } else {
              setRecentChats([]);
              setUsersData([]);
            }
  ...
  // Automatically select the first chat when recentChats updates
  useEffect(() => {
        ...
        <div className="flex px-0 bg-white justify-between items-center border-b py-4">
          ...
          >
            ...
            >
              {isSmallScreen ? (
                chatLocation ? (
                  <LuPencil className={`text-xl ${chatLocation ? "text-white" : ""}`} />
                ) : (
                  <img src={Notifications} alt="Notifications" />
                )
              ) : (
                ...
                recentChats.length > 0 ? (
                  recentChats.map((usr) => (
                    <AllUsers
                      ...
                      recentMessage={usr.data.recentMessage}
                      time={usr.data.formattedTime} // Use formattedTime here
                      isActive={selectedChatId === usr.id}
                      isSelected={selectedChatId === usr.id}
                      onClick={() => {
                        setSelectedChatId(usr.id);
                        setIsSheetOpen(true);
                      }}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2 items-center">
                    <img
                      src={
                        recentChats.find(chat => chat.id === selectedChatId)?.data?.otherImage ||
                        ""
                      }
                      alt="User"
                      className="w-[59px] h-[59px] rounded-full"
                    ...
                    <div className="flex flex-col items-start">
                      <h1 className="font-bold">
                        {recentChats.find(chat => chat.id === selectedChatId)?.data?.otherName}
                      </h1>
                      <p>{recentChats.find(chat => chat.id === selectedChatId)?.data?.formattedTime}</p>
                    </div>
                  </div>
                </div>
                <div>
