...
const UsersChat = ({ otherUserId, userImg, username, time, onSendMessage, otherName, otherImage }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
      ...
      <div
        ref={messagesContainerRef}
        className="flex flex-col flex-1 overflow-y-auto bg-white p-4"
      >
        <div className="flex flex-col gap-2">
          {messages.length > 0 ? (
            messages.map((message) => {
              const isCurrentUser = message.data.senderId === userID;
              return (
                <div
                  key={message.id}
                  className={`flex flex-col ${isCurrentUser ? 'items-end' : 'items-start'}`}
      ...
      {/* Input Area */}
      <div className="sticky bottom-0 bg-white p-4 flex items-center gap-1">
        <div className="bg-gray-100 w-auto flex-grow rounded-3xl">
          <textarea
            placeholder="Enter the message"
            value={newMessage}
            onChange={(e) => setNewMessage(e.target.value)}
            onKeyPress={handleKeyPress}
            className="p-3 outline-none bg-transparent w-full resize-none h-12"
          />
        </div>

        <div
          onClick={handleSend}
          className="bg-black flex-grow-0 rounded-full p-2 flex justify-center items-center cursor-pointer"
        >
          <img src={newArrow} alt="Send" className="w-6 h-6" />
        </div>
      </div>
    </div>
  );
