...
const AllUsers = ({ otherName, otherImage, otherID, recentMessage, time, hasUnread, isActive, isSelected, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`relative p-2 cursor-pointer border-b border-gray-300 transition-colors duration-300 ${
        isSelected
          ? 'bg-[#ECF5FE] border-l-2 border-black'
          : isActive
            ? 'bg-[#f5f5f5]' // Changed from 'bg-brown-500' to a neutral color
        ...
        <div className='flex mt-2 gap-2'>
          ...
          <div className='mb-1'>
            ...
            <div className='mb-1 flex items-center'>
              <h2 className={`tracking-tighter ${isActive ? 'font-normal text-gray-600' : 'font-semibold text-gray-800'}`}>
                {recentMessage ? (recentMessage.length > 22 ? `${recentMessage.slice(0, 22)}...`  : recentMessage ) : 'No messages yet'} 
              </h2>
            </div>
            <p className='text-gray-400 text-sm'>{time}</p>
          </div>
